...
It is good practice to start with an exploratory data analysis (EDA) to understand the structure of the data, check for missing values, and identify any potential issues before performing Key Driver Analysis (KDA). `YouAnalyser` provides conventient functions for EDA, all starting with *eda_*: `eda_summary()` and `eda_correlation()`. These functions can help you get a quick overview of your data and identify any necessary preprocessing steps. But first, we need some data!
...
```{r include = FALSE}
example_data <- YouAnalyser::bkw_raw
```

`example_data` contains our outcome variable `F600` and 14 predictor variables `F800_1` to `F800_14`. To get more information about this included dataset, you can run `?YouAnalyser::bkw_raw` in the R console:
...
if (FALSE) {
  ...
  saveRDS(kda, file = "./vignettes/kda_dominanceAnalysis.rds")
} else {
