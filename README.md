...
- You can find the documentation for each function by calling
  `?function_name` in your R console (e.g., `?eda_summary`).
...
e.g. `vignette("dp", package = "YouAnalyser")`), which provide
step-by-step guides on how to use the various functions for data
processing and other convenient tasks.

`YouAnalyser` assumes that your data comes in the `haven::labelled()`
...
- `dp_copy_codebook_template()`: Copy a codebook template to a specified
  file path
...
- `ya_choose_file_path()`: Choose an existing directory and define a new
  file in this directory, returning the full file path
- `ya_save_plot()`: Save a plot to JPEG

Special consideration should be given to `ya_setup_folder_structure()`,
...
The `ya_setup_folder_structure()` function is used in workflows. See
`vignette("kda", package = "YouAnalyser")` for an example of how to use
it in a workflow.
...
in the package (use e.g. `vignette("eda", package = "YouAnalyser")`),
...
`eda_summary()` and `eda_correlation()`. The former provides a summary
of the data, including descriptive statistics and missing values, while
the latter calculates and visualizes correlations between variables.
...
#> F800_8   | 4.59 | 1.20 | [1.00, 7.00] | 4.00, 5.00 |    -0.06 |     0.36 | 1171 |         0
```
...
<figure>
<img src="man/figures/eda_summary_browserOutput_screenshot.png"
alt="Summary Table as a Browser Output" />
<figcaption aria-hidden="true">Summary Table as a Browser
Output</figcaption>
</figure>
...
<img src="man/figures/README-eda_correlation-example-1.png" width="100%" />

There are many more options for `correlation_type`. To see all available
options, call `?correlation::correlation` and read the details section.
...
in the package (use e.g. `vignette("kda", package = "YouAnalyser")`),
which provide step-by-step guides on how to use the various functions
for KDA.
...
e.g. `vignette("kda", package = "YouAnalyser")`), which provide
...
save these plots using the `ya_save_plot()` function, which allows you
to specify the file path and dimensions of the saved plot.
