...
* `dp_label_automatically()`: Automatically label a data frame based on its values (useful for filling in missing labels if you do not want to use `dp_convert_to_labelled()` with a codebook - use with caution!)
* `dp_inspect_codebook()`: Inspect the codebook of a labelled data frame (useful to check whether labelling worked as expected)
* `dp_zap_missings()`: Remove missing values and their value labels from a labelled data frame.
